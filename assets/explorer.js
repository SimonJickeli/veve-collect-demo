...
(function () {
  ...
  function cost(c) { var sRaw = stackr(c.name) || 0, vRaw = c.floor || 0; var k = window.saneStackrFloor ? window.saneStackrFloor(sRaw, vRaw) : sRaw; if (k > 0) return { v: k, s: 'StackR ⛓' }; var vv = window.saneVeveFloor ? window.saneVeveFloor(vRaw, sRaw) : vRaw; if (vv > 0) return { v: vv, s: 'VeVe ask 💠' }; if (!c.blind && c.price > 0) return { v: c.price, s: 'drop' }; return null; }
